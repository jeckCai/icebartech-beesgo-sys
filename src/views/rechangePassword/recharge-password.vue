<template>
  <div class="wrap">
    <div class="login-wrap">
      <div class="login-title">蜜蜂GO后台管理系统-后台重置密码</div>
...
export default {
  ...
  methods: {
    login() {
      if (!this.oldPassword || !this.password || !this.confirmNewPWD) {
        this.$message.warning("请填写完整修改密码信息");
        return;
      }
      let param = {
      ...
      formPost("UPDUSERPWD", param).then(res => {
        if (!res) return;
        this.$message({
          type: "success",
          message: "密码修改成功!"
