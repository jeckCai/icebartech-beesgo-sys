      ...
      >
        ...
        <el-form-item label="商品图" prop="imageUrls" required>
          <template v-if="formData.imageUrls.length!==0">
            <el-col style="width:130px;height:100px;" :span="2" v-for="(item,i) in formData.imageUrls" :key="i">
              <div class="remove addImg">
                <uploadOne :clear="false" :keyIndex="i" :isShowImg="true" :imgUrl.sync="item.fileUrl||item.imageUrl" @deleteImg="deleteFalseImg" @upLoadSuccess="v=>{item.fileKey=v.filekey,item.imageKey=v.fileKey}"></uploadOne>
                <!-- <img
                ...
                <!-- <span @click="remove(i)">×</span><i class="el-icon-close deleteIcon" @click="remove(i)"></i> -->
              </div>
            </el-col>
          </template>
        ...
        <el-form-item label="选择商品属性" required label-width="120px">
          ...
          >
            ...
            <!-- <el-checkbox-group v-model="checkSize"  @change="handleCheckedSizeChange">
                  <el-checkbox :label="items.attributeValue" disabled="true" v-for="(items,index) in item.valueList" :key="index"></el-checkbox>
            </el-checkbox-group>-->
          </el-form-item>
        </el-form-item>
          ...
          <el-table
            ...
            <el-table-column prop="attributeValue" :label="item.modelName" width="120" v-for="(item,i) in tableDatas" :key="i">
               <template slot-scope="scope"> 
                  <el-select  v-model="scope.row.attributeValueIds[i]" placeholder="请选择" :disabled="scope.row.id?true:false">
                      <el-option v-for="items in item.valueList" :key="items.id" :label="items.attributeValue" :value="items.id"></el-option>
                  </el-select>
                 </template>
            </el-table-column>
            <el-table-column
            ...
            <el-table-column
             ...
             <el-table-column prop label="操作" width="280px">
              <template slot-scope="scope">
                <el-button type="danger" :disabled="scope.row.id?true:false" size="mini" @click="handleDeleteItem(scope.$index, formData.models1)">删除</el-button>
...
export default {
  ...
  data() {
      return { 
        ...
        modelSizeCheck:[{name:'S',checked:true},{name:'M',checked:false},{name:'L',checked:false}], // 商品尺码选择列表
        modelColor:['黄色','红色','白色'], // 商品颜色列表
        modelColorCheck:[{name:'黄色',checked:true},{name:'红色',checked:false},{name:'白色',checked:false}], // 商品颜色选择列表
        checkSize:[], // 选中的尺码
        checkColor:[], // 选中的颜色
        firghtList:[], // 邮费模板列表
        formData:{
        ...
        rules: { 
          ...
          checkSize: [
             { type: 'array', required: true, message: '请至少选择一个尺码', trigger: 'change' }
          ], 
          frightId: [
  ...
  methods: {
    renderHeaderStock(h, { column, $index }){//库存数量
    ...
    handleAddeteItem(){
      ...
      this.formData.models1.push({ attributeValueIds,goodsSkuKey:"",markingPrice:"",salePrice:"",})
    },
    // 删除
    ...
    //     let tempArr=new Array(tableDatas.length); 
    ...
    submitForm(formName) {
      ...
      let attributeIds=[]
        ...
        if(this.edit==0){
         ...
         let arrs=[]
        let stockParams=this.formData.models1.map(({salePrice,attributeValueIds,markingPrice,stock,goodsSkuKey,id})=>{
          arrs.push(attributeValueIds)
          if(id){
            return {salePrice,attributeValueIds,markingPrice,stock,goodsSkuKey,id}
          }else{
            return {salePrice,attributeValueIds,markingPrice,stock,goodsSkuKey}
          }
        })
        let flag=false;
        ...
        stockParams.forEach(item=>{
          if(!item.markingPrice||!item.salePrice||!item.stock||item.attributeValueIds.length==0){
            flag=true;
          }
          if(!item.goodsSkuKey){
