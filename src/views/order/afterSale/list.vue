 ...
 * @LastEditTime : 2019-12-25 19:06:23
...
<template>
        ...
        <el-table
            ...
            <el-table-column label="操作" width="300px">
                <template slot-scope="scope">
                    ...
                    <el-button type="success" size="mini" v-if="(scope.row.isCheck == 'n' && scope.row.type != 'CHANGE_PRODUCT') ||(scope.row.type == 'CHANGE_PRODUCT' && scope.row.status == 'WAIT_CHECK') " @click="handleCheck(scope.row,'pass')">通过</el-button>
                    <el-button type="danger" size="mini" v-if="(scope.row.isCheck == 'n' && scope.row.type != 'CHANGE_PRODUCT') || (scope.row.type == 'CHANGE_PRODUCT' && scope.row.status == 'WAIT_CHECK') " @click="handleCheck(scope.row,'reject')">拒绝</el-button>
                </template>
             </el-table-column>
       </el-table>
      </div>
    ...
    <!-- 审核通过 -->
     <el-dialog title="同意" :visible.sync="dialogPass">
        ...
        <el-form-item label="退款金额" prop="checkPrice">
          <el-col :span="10">
             <el-input type="text" v-model="orderPass.checkPrice"  placeholder="请输入"></el-input>
          </el-col>
        </el-form-item>
        </el-form>
...
export default {
    ...
    mounted(){
       ...
       if(path == '/brandQuickSale'){
           this.queryData.orderType = 'QUICK_ORDER';
       }else{
           this.queryData.orderType = 'NORMAL_ORDER';
       }
       this.getList()
    },
    methods:{
        handleView(id){
            this.$router.push('afterSaleDetail?id='+id);
        },
        ...
        handlePass(formName){
             this.$refs[formName].validate((valid) => {
                if (valid) {
                    this.orderPass.refundPrice = this.orderPass.checkPrice;
        ...
        getList(reset) {
                ...
                httpPost('afterSalePage',this.queryData).then(res=>{
                    if (res.code == 200) {
